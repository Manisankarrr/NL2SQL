...
def format_insert_result(result: ExecutionResult, entities: Optional[ExtractedEntities] = None) -> dict:
    ...
    if result.affected_rows == 0:
        return {"message": "No appointment was created. The customer name may not exist in the system.", "affected_rows": 0}
    ...
    return {"message": f"Appointment booked for {name}{date_str}{time_str}.", "affected_rows": result.affected_rows}
...
def format_update_result(result: ExecutionResult, entities: Optional[ExtractedEntities] = None) -> dict:
    ...
    if result.affected_rows == 0:
        return {"message": "No matching appointments found to update. Check the name and date.", "affected_rows": 0}
...
def format_delete_result(result: ExecutionResult, entities: Optional[ExtractedEntities] = None) -> dict:
    ...
    if result.affected_rows == 0:
        return {"message": "No matching appointments found to cancel.", "affected_rows": 0}
    n = result.affected_rows
...
def format_error(error_message: str, sql: Optional[str] = None) -> dict:
    return {"message": f"I couldn't process that request. {error_message}", "error": True, "sql": sql, "table": [], "count": 0, "affected_rows": 0}


def format_response(intent: SQLIntent, result: ExecutionResult, user_query: str, entities: Optional[ExtractedEntities], sql: str) -> dict:
